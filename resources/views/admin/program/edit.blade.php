...
@section('conten')
<form method="POST" action="/programAdmin/{{ $program->nama }}" class="p-4 md:p-5" enctype="multipart/form-data">
    @method('put')
    @csrf
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="nama" id="nama" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value="{{ old('nama', $program->nama) }}" required />
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="deskripsi" id="floating_password" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " value="{{ old('deskripsi', $program->deskripsi) }}" required />
        ...
        <div class="relative z-0 w-full mb-5 group">
        <input id="tujuan" type="hidden" name="tujuan" value="{{ old('tujuan', $program->tujuan) }}">
        <trix-editor input="tujuan"></trix-editor>    
        </div>
        <div class="relative z-0 w-full mb-5 group">
            <input type="hidden" name="oldCover" value="{{ $program->cover }}">
            @if($program->cover)
            <img src="{{ asset('storage/' . $program->cover) }}" class="img-preview max-w-[200px]">
            @else
        ...
        @enderror
        <div class="relative z-0 w-full mb-5 group">
            @foreach ($program->img as $img)
            <input type="hidden" name="oldImage[]" value="{{ $img }}" multiple>
            @endforeach
            <label class="block mb-2 text-sm font-medium text-gray-900" for="user_avatar">Image</label>
            <input class="@error('cover') is-invalid @enderror block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none " aria-describedby="user_avatar_help" id="img" name="img[]" type="file" multiple>   
    ...
    <a href="/programAdmin" class="bg-red-500 text-white p-2 rounded-xl ml-4">Cancel</a>
