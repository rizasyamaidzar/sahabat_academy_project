...
<h2 class="text-4xl font-bold mb-5 text-blue-950">{{ $title }} Player</h2>
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <div class="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-gradient-to-tl from-purple-500 to-pink-400">
        <div class="m-5">
            ...
            <h2 class="text-2xl font-bold text-white text-center">{{ $jumlahcowo }}</h2>
        </div>
            ...
            <tr class="bg-white border-b hover:bg-gray-50 text-center ">
                ...
                <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                    ...
                    <div class="ps-3">
                        ...
                        <div class="font-normal text-gray-500">{{ $d->username }}</div>
                    </div>  
                </th>
                <td class="px-6 py-4">
                {{ $d->jenis_kelamin }}
                ...
                <td class="px-6 py-4">
                    ...
                    <a href="{{ $d->user->id }}" class="mx-auto bg-{{ $d->user->status ? 'green-500' : 'red-500' }} text-white p-2 rounded-lg">
                            {{ $d->user->status ? 'Aktif' : 'Tidak Aktif' }}
                        </a>                        
                    </div>
                    </div>
                </td>
                <td class="px-6 py-4 text-center">
                    <a href="/dashboard/{{ $link }}/{{ $d->nama  }}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">View user</a>
                </td>
                <td class="px-6 py-4 text-center">
...
<div class="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-gradient-to-tl from-pink-500 to-purple-400">
        <div class="m-5">
            ...
            <h2 class="text-2xl font-bold text-white text-center">{{ $jumlahcewe }}</h2>
        </div>
        <label for="table-search" class="sr-only">Search</label>
        <div class="relative">
            <div class="absolute inset-y-0 rtl:inset-r-0 start-0 flex items-center ps-3 pointer-events-none">
